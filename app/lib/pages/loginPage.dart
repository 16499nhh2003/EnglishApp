...
//show dialog nếu email không đunhs định dạng
  ...
  //show dilog khi email không tồn tại trong database
  void _showEmailNotExistsDialog() {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              "Email này không tồn tại trong hệ thống. Vui lòng kiểm tra lại hoặc đăng kí để tiếp tục"),
          actions: <Widget>[
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  //show dialog password not empty
  void _showPasswordNotMatchDialog() {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text("Mật khẩu không đúng"),
